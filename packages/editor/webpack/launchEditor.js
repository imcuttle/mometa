...
function launchEditor(fileName, lineNumber, colNumber) {
  ...
  if (!(Number.isInteger(lineNumber) && lineNumber > 0)) {
    throw new Error(`lineNumber ${lineNumber} is invalid`)
  }
  ...
  if (!editor) {
    ...
    throw new Error('Could not open ' + path.basename(fileName) + ' in the editor.')
  }

  if (editor.toLowerCase() === 'none') {
    throw new Error('Could not open ' + path.basename(fileName) + ' in the editor: none.')
  }
